...
let wire1=i[0].split(",").map(i=>i.match(/^([RLUD])([0-9]+)$/)).map(i=>move(i[1],parseInt(i[2])))
x=0;y=0;
let wire2=i[1].split(",").map(i=>i.match(/^([RLUD])([0-9]+)$/)).map(i=>move(i[1],parseInt(i[2])))
...
//Part 2
let dist=0;
move=(d,s)=>{let dx=0;let dy=0;if(d=="R"){dx=s}if(d=="L"){dx=-s}if(d=="U"){dy=s}if(d=="D"){dy=-s}y+=dy;x+=dx;dist+=Math.abs(dy)+Math.abs(dx);return[x-dx,y-dy,x,y,dist]}
x=0;y=0;
...
cross=(a1,a2)=>{if(Math.min(a1[0],a1[2])<Math.max(a2[0],a2[2])&&Math.min(a2[0],a2[2])<Math.max(a1[0],a1[2])&&Math.min(a1[1],a1[3])<Math.max(a2[1],a2[3])&&Math.min(a2[1],a2[3])<Math.max(a1[1],a1[3])){ return a1[0]==a1[2]?a1[4]-Math.abs(a1[3]-a2[1])+a2[4]-Math.abs(a2[2]-a1[0]):a2[4]-Math.abs(a2[3]-a1[1])+a1[4]-Math.abs(a1[2]-a2[0]);}return 999999999999;}
wire2.reduce((res,arc)=>Math.min(wire1.reduce((lres,larc)=>Math.min(cross(arc,larc),lres), 999999999),res), 999999999)